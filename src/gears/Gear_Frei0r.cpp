...
_sizeY(0)
{
  ...
  _handle = dlopen("gears/frei0r/distort0r.so", RTLD_LAZY);
  ...
  *(void**) (&f0r_update) = dlsym(_handle, "f0r_update");
  ...
  //get infos  
  (*f0r_get_plugin_info)(&_inverterInfo);

  std::cout << "Frei0r plugin: " << _inverterInfo.name << std::endl;
  std::cout << "author: " << _inverterInfo.author << std::endl;        /**< The plugin author                                */
  std::cout << "type: " << _inverterInfo.plugin_type << std::endl;     /**< The plugin type (source or filter)               */
  std::cout << "colormodel: " << _inverterInfo.color_model << std::endl;     /**< The color model used                             */
  std::cout << "version: " << _inverterInfo.frei0r_version << "." << _inverterInfo.major_version << "." << _inverterInfo.minor_version << std::endl;  /**< The frei0r major version this plugin is built for*/
  std::cout << "num_params: " << _inverterInfo.num_params << std::endl;      /**< The number of parameters of the plugin           */
  std::cout << "details: " << _inverterInfo.explanation << std::endl;     /**< An optional explanation string (can be 0)    */
  
  for (int i=0; i<_inverterInfo.num_params; ++i)
  {
...
void Gear_Frei0r::runVideo()
{
  ...
  for (int i=0,k=0; i<_inverterInfo.num_params; ++i)
  {
    f0r_param_info param_info;
    (*f0r_get_param_info)(&param_info, i);
    switch (param_info.type)
