...
namespace frei0r
{
  ...
  public:
    ...
    virtual void update_l(double time, const uint32_t* inframe, uint32_t* outframe) = 0;
  ...
  class source : public fx
    ...
    public:
      virtual bool is_filter(){return false;}
      
      virtual void update_l(double time_, const uint32_t* inframe, uint32_t* outframe_)
	{
	  time = time_;
	  out = outframe_;
	  update();
  ...
  class filter : public fx
    ...
    public:
      virtual bool is_filter(){return true;}
      
      virtual void update_l(double time_, const uint32_t* inframe, uint32_t* outframe)
	{
	  time = time_;
	  out = outframe;
	  in = inframe;
...
{
  static_cast<frei0r::fx*>(instance)->update_l(time, inframe, outframe);
}
