...
typedef struct squareblur_instance
{
  ...
  unsigned int acc_width; /* = width+1 */
  unsigned int acc_height; /* = height+1 */
  uint32_t *mem; /* memory accumulation matrix of uint32_t (size = acc_width*acc_height*4) */
  uint32_t **acc; /* accumulation matrix of pointers to 4 consecutive uint32_t in mem (size = acc_width*acc_height) */
...
void update_summed_area_table(squareblur_instance_t *inst, const uint32_t *src)
{
  ...
  register uint32_t *iter_acc;
  register unsigned int i, x, y;

  uint32_t acc_buffer[4]; /* accumulation buffer */

  unsigned int row_width;
  unsigned int width, height;
  unsigned int cell_size = 4 * sizeof(uint32_t);
  ...
  memset(iter_acc, 0, row_width * sizeof(uint32_t));
  iter_acc += row_width;
  
  // Process second row.
  memset(acc_buffer, 0, cell_size);
  memset(iter_acc,    0, cell_size); // first column is void
  iter_acc += 4;
  for (x=1; x<width; ++x)
  {
    for (i=0; i<4; ++i)
      *iter_acc++ = (acc_buffer[i] += *iter_data++);
  }
  ...
  {
    // Copy upper line.
    memcpy(iter_acc, iter_acc - row_width, row_width * sizeof(uint32_t));
...
void f0r_get_plugin_info(f0r_plugin_info_t* inverterInfo)
{
  ...
  inverterInfo->author = "Mathieu Guindon, Julien Keable, Jean-Sebastien Senecal";
...
f0r_instance_t f0r_construct(unsigned int width, unsigned int height)
{
  ...
  inst->acc_width = width+1; inst->acc_height = height+1;
  inst->kernel = 0.0;
  /* allocate memory for the summed-area-table */
  inst->mem = (uint32_t*) malloc(inst->acc_width*inst->acc_height*4*sizeof(uint32_t));
  inst->acc = (uint32_t**) malloc(inst->acc_width*inst->acc_height*sizeof(uint32_t*));
  /* point at the right place */
  for (i=0; i<inst->acc_width*inst->acc_height; ++i)
    inst->acc[i] = &inst->mem[i * 4];
                ...
                const uint32_t* inframe, uint32_t* outframe)
{
  ...
  unsigned int kernel_size = (unsigned int) (inst->kernel * MAX(width, height) / 2);
  ...
  if (kernel_size <= 0)
  {
    ...
    memcpy(outframe, inframe, inst->width*inst->height*sizeof(uint32_t));
  }
  else
  {
    assert(inst->acc);
    ...
    for (y=0;y<height;y++)
    {
      for (x=0;x<width;x++)
      {
        ...
        memcpy(sum, acc[y1_offset + x1], 4*sizeof(uint32_t));
