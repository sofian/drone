...
// XXX bigbig hack
#define gimp_rgb_to_hsl_int rgb_to_hsl_int
#define gimp_hsl_to_rgb_int hsl_to_rgb_int
#define gimp_rgb_to_hsv_int rgb_to_hsl_int
#define gimp_hsv_to_rgb_int hsl_to_rgb_int

#include <cstring>

// XXX this is defined in Utils.h
...
gimp_composite_blend_any_any_any_generic (GimpCompositeContext *ctx)
{
  ...
  guint bytes = 4; // gimp_composite_pixel_bpp[ctx->pixelformat_A];
...
gimp_composite_darken_any_any_any_generic (GimpCompositeContext * ctx)
{
  ...
  guint bytes1 = 4; // gimp_composite_pixel_bpp[ctx->pixelformat_A];
  guint bytes2 = 4; // gimp_composite_pixel_bpp[ctx->pixelformat_B];
          ...
          if (src2[b] > 128)
            {
              ...
              dest[b] = (guchar) MIN (255 - (tmp >> 8), 255u);
            }
          else
            {
              tmp = (gint) src1[b] * ((gint) src2[b] << 1);
              dest[b] = (guchar) MIN (tmp >> 8, 255u);
            }
        }
...
gimp_composite_softlight_any_any_any_generic (GimpCompositeContext * ctx)
{
  ...
  guint bytes1 = 4; // gimp_composite_pixel_bpp[ctx->pixelformat_A];
  guint bytes2 = 4; // gimp_composite_pixel_bpp[ctx->pixelformat_B];
  const guint has_alpha1 = 4; // gimp_composite_pixel_alphap[ctx->pixelformat_A];
  const guint has_alpha2 = 4; // gimp_composite_pixel_alphap[ctx->pixelformat_B];
  const guint alpha = (has_alpha1 || has_alpha2) ? MAX(bytes1, bytes2) - 1 : bytes1;
...
gimp_composite_dissolve_any_any_any_generic (GimpCompositeContext * ctx)
{
  ...
  gint sb = 4; // gimp_composite_pixel_bpp[ctx->pixelformat_B];
  ...
  guchar *dest = ctx->D;
  guchar *src = ctx->B;
  guint has_alpha = 4; // gimp_composite_pixel_alphap[ctx->pixelformat_B];
...
//   }
  db = 4; // gimp_composite_pixel_bpp[ctx->pixelformat_D];
  ...
  ctx->combine = COMBINE_INTEN_A_INTEN_A;
...
{
  ctx->D = ctx->B;
  ctx->combine = COLOR_ERASE_INTEN;// (gimp_composite_pixel_alphap[ctx->pixelformat_A] && gimp_composite_pixel_alphap[ctx->pixelformat_B])
//     ? COLOR_ERASE_INTEN
//     : 0;
}
...
gimp_composite_scale_any_any_any_generic (GimpCompositeContext * ctx)
{
  ...
  guchar *dest = ctx->D;
  guint length = ctx->n_pixels;
  guint bytes1 = 4;// (ctx->pixelformat_A == GIMP_PIXELFORMAT_V8) ? 1
