...
public class CoreApplication implements Application, ExecutionListener {
	...
	/**
	 * Reduce the count of executing models by one. If the number of executing
	 * models drops to zero, then notify threads that might be waiting for this
	 * event.
	 * 
	 * @param manager
	...
	/**
	 * Reduce the count of executing models by one. If the number of executing
	 * models drops ot zero, then notify threads that might be waiting for this
	 * event.
	 * 
	 * @param manager
	 *            The manager calling this method.
...
//	}
	...
	/**
	 * Read a Configuration from the URL given by the specified string. The URL
	 * may absolute, or relative to the Ptolemy II tree root, or in the
	 * classpath. To convert a String to a URL suitable for use by this method,
	 * call specToURL(String).
	 * <p>
	 * If there is an _applicationInitializer parameter, then instantiate the
	 * class named by that parameter. The _applicationInitializer parameter
	 * contains a string that names a class to be initialized.
	 * <p>
	 * If the configuration has already been read in, then the old configuration
	 ...
	 * @exception Exception
	 *                If the configuration cannot be opened, or if the contents
	 ...
	 * resource accessible to the class loader, which uses the classpath to find
	 ...
	 * @param commandFlags
	 *            Command-line options that are either present or not.
					...
					try {
						...
						if (!configurationName.equals("jxta")) {
							...
							if (specificationURL
								...
								// Avoid rereading the configuration, which will
								// result
								// in the old configuration being removed, which
								// exits the app.
...
//        doc.identifier.setExpression(introURL.toExternalForm());
			...
			if (_expectingClass) {
				...
				if (!arg.startsWith("-")) {
					...
					if (_configuration != null) {
						...
						// FIXME: If the -run option was given, then start a
						// run.
						// FIXME: If the -fullscreen option was given, open full
						// screen.
			...
			while (effigies.hasNext()) {
				...
				if (effigy instanceof PtolemyEffigy) {
					...
					if (attribute instanceof Settable) {
						 ...
						 * ((Settable)attribute).setExpression(value); if
						 * (attribute instanceof Variable) { // Force evaluation
					...
					if (model instanceof CompositeActor) {
						...
						if (director != null) {
							...
							if (attribute instanceof Settable) {
								 ...
								 * if (attribute instanceof Variable) { // Force
								 * evaluation so that listeners // are notified.
								 * ((Variable)attribute).getToken(); }
								 */
							}
						}
	...
	/**
	 * Look for directories that contain files named configuration.xml and
	 * intro.htm.
	 */
	class ConfigurationFilenameFilter implements FilenameFilter {
		/**
		 * Return true if the specified file names a directory that contains a
		 * file named configuration.xml and a file named intro.htm
		 * 
		 * @param directory
		 *            the directory in which the potential directory was found.
		 * @param name
